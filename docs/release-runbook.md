...
1. **Decide the version.** SemVer; for example `3.0.0`. Confirm the
...
```bash
JAVA_HOME=<JDK 21+> ./scripts/verify_release_rehearsal.sh 3.0.0
```

The rehearsal validates SemVer, requires that `v3.0.0` does not already exist locally, verifies the
runbook recovery markers, and runs API compatibility plus release-SBOM checks with
`-PreleaseVersion=3.0.0`. It never reads publication credentials, tags a commit, uploads artifacts,
