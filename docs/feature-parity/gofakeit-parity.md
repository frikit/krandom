...
|-----------------------------------------------------------------------------------------|----------------|---------------------|-----------------------------------------------------------------------------------------|
| Reconcile GoFakeit alias names for core providers (identity/address/network/date/text)  | Partial        | Explicit gap        | Core behavior exists; exact Go naming wrappers remain a compatibility-layer decision.   |
...
| Template compatibility (`Generate("{function}")` + function lookup registry)            | Partial        | Explicit gap        | Existing template/string generators provide base; provider-token registry is still open. |
| Structured output providers (CSV/JSON/XML/SQL/Markdown/fixed-width)                     | Partial        | Explicit gap        | CSV/JSON/XML/SQL/JSON Schema exist; Markdown and fixed-width exports are not targeted.  |
