...
`fake` is a Rust library for generating realistic-looking fake data for testing, seeding databases, and populating fixtures. It covers a broad range of domains — addresses, names, internet data, finance, dates, barcodes, colours, and more — through a unified trait-based API.
...
All faker types are zero-sized unit structs (or tuple structs where a parameter is needed). They live under `fake::faker::<category>::<locale>`. The canonical path for English is `fake::faker::<category>::en::<Type>`.
