...
**Enums:** `TLDType` (CCTLD, GTLD, GEOTLD, UTLD, STLD), `PortRange` (ALL, WELL_KNOWN, REGISTERED, DYNAMIC), `MimeType` (TEXT, IMAGE, AUDIO, VIDEO, APPLICATION), `Layer` (APPLICATION, TRANSPORT,
NETWORK, DATA_LINK), `URIScheme` (HTTP, HTTPS, FTP, FTPS, SFTP, WS, WSS).
...
mimesis `Schema` + `Field` generates declarative heterogeneous records. krandom has `Generator.generateList(n)` (scalar, one type) and `ObjectGenerator<T>` (reflection-based POJO). No equivalent to
`Schema` for structured multi-field records exists in krandom.
