...
- [x] Resolve or upgrade the Central publication plugin before its deprecation becomes a Gradle 10 error.
...
- [x] Wrapper verification fails with a deliberately wrong checksum.
...
NMCP 1.6.1 remains the latest upstream release. The build now uses its supported manual aggregation
mode instead of the convenience settings plugin, eliminating the deprecated project-notation call
while keeping the seven published modules explicit.
...
- [x] Publish the exact supported-constraint table from code facts.

**Tests**

- [x] Add red/green cases for every supported annotation, boundary, composition, and nullability interaction.
...
- [x] Validate generated objects through Hibernate Validator across many deterministic seeds.
...
- [x] Prove adding an unrelated object field does not perturb existing named fields where the contract promises stability.
- [x] Prove locale, clock, provider data, and safety-policy changes appear in the recipe.
...
- [x] Validate registered keys and data arrays uniformly.
...
- [x] Locale fallback and registry validation are uniform across old and new provider families.
