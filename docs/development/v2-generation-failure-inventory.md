...
| GFI-11 | `FieldGeneratorResolver` unsupported type branch | Direct and nested unsupported types use the central policy; strict reports context and lenient returns the type default | Strict/lenient failure | Complete; the erased nested `Object` bridge was removed after recursive type resolution replaced it |
