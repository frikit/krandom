...
| `DataRegistryContext` | Per-configuration override/fallback | 19 locale families: names, gender/title/suffix/profession, location, national IDs, weather, measurement, financial terms, restaurant types, hobbies, nationalities, pronouns, and blood types | Does not yet cover the other global registries |
...
Weather, measurement, financial terms, restaurant types, hobbies, nationalities, pronouns, and
blood types now have validated context registration, locale fallback, and snapshot keys in
`DataRegistryContext`; their generators resolve vocabulary from `GeneratorConfig` rather than
directly from process-wide state. The remaining vocabulary families will follow the same boundary
