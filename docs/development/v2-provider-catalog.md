...
| `DataRegistryContext` | Per-configuration override/fallback | 16 locale families: names, gender/title/suffix/profession, location, national IDs, weather, measurement, financial terms, restaurant types, and hobbies | Does not yet cover the other global registries |
...
Weather, measurement, financial terms, restaurant types, and hobbies now have validated context
registration, locale fallback, and snapshot keys in `DataRegistryContext`; their generators resolve
vocabulary from `GeneratorConfig` rather than directly from process-wide state. The remaining
vocabulary families will follow the same boundary before the legacy static registries are deprecated.
