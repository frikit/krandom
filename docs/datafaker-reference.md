...
DataFaker is the **official successor to JavaFaker**. It is a JVM library (Java, Kotlin, Groovy) for generating realistic fake data across hundreds of domains. It was created as a modernised fork of `java-faker` with updated dependencies, expanded provider coverage (200+ providers), GraalVM native-image support, and a substantially improved API.
...
new Faker(new Locale("zh", "CN"))  // Chinese (Simplified)
new Faker(new Locale("ja"))        // Japanese
new Faker(new Locale("ko"))        // Korean
new Faker(new Locale("ru"))        // Russian
new Faker(new Locale("es", "MX"))  // Spanish (Mexico)
new Faker(new Locale("pt", "BR"))  // Portuguese (Brazil)
```
...
faker.letterify("???-???")     // "kxm-pty"
faker.bothify("##??##")        // "27kq91"
faker.regexify("[A-Z]{3}\\d{4}") // "KXP7214"
```
...
faker.expression("#{Address.city}")           // "Los Angeles"
faker.expression("#{numerify '###-####'}")    // "492-3718"
faker.expression("#{letterify '???'}")        // "kxm"
faker.expression("#{regexify '[A-Z0-9]{8}'}")// "K3PQ8WR1"
faker.expression("#{options.option 'a','b','c'}")  // "b"
faker.expression("#{date.birthday 'yy','MM','dd'}") // "85,03,21"
```
...
faker.addUrl(Locale.ENGLISH, new URL("https://example.com/en.yml"));
```
...
faker.stream(() -> faker.name().fullName())
    .limit(100)
    .generate()
...
faker.unique().fetchFromYaml("name.first_name", Locale.ENGLISH);
...
faker.expression("#{Address.streetAddress}")
// "123 Main St"

faker.expression("#{numerify '###-####'}")
// "492-3718"

faker.expression("#{letterify '???-???'}")
// "kxm-pty"

faker.expression("#{regexify '[A-Z]{2}[0-9]{4}'}")
// "KX2914"

faker.expression("#{bothify '##??##'}")
// "27kq91"

faker.expression("#{options.option 'red','green','blue'}")
// "green"

faker.expression("#{Internet.emailAddress 'john.doe'}")
// "john.doe@yahoo.com"

faker.expression("#{date.birthday 'yy','MM','dd'}")
// "85,03,21"
```
