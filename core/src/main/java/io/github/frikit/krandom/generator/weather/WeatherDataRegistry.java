...
/**
 * Global registry mapping locales to {@link WeatherDataProvider} instances.
 *
 ...
 * {@code null}.
 */
public final class WeatherDataRegistry {

    private static final ConcurrentHashMap<String, WeatherDataProvider> REGISTRY =
     ...
     */
    public static @Nullable WeatherDataProvider forLocale(@Nullable Locale locale) {
        if (locale == null) {
            return null;
        }
        String lang = locale.getLanguage();
        String country = locale.getCountry();
        if (!country.isEmpty()) {
            WeatherDataProvider exact = REGISTRY.get(lang + "_" + country);
            if (exact != null) {
                return exact;
            }
        }
