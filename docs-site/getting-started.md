...
The current version is `2.4.0`. Latest version is always shown on
...
Import `io.github.frikit:krandom-bom` in Gradle or Maven and omit versions from individual kRandom modules. Use explicit `2.4.0` versions instead when consuming only one artifact without the BOM.
