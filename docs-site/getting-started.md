...
The current version is `2.5.0`. Latest version is always shown on
...
dependencies {
    implementation(platform("io.github.frikit:krandom-bom:2.5.0"))
...
Import `io.github.frikit:krandom-bom` in Gradle or Maven and omit versions from individual kRandom modules. Use explicit `2.5.0` versions instead when consuming only one artifact without the BOM.
